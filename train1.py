...
class BasicBlock_disout(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = self.dropout5(shortcut)
        x += shortcut
        x = self.normalizer_fn3(x)
        x = self.activation_fn(x)
...
def main(data_dir,FLAG):
    ...
    wlp300_dataloader = DataLoader(dataset=wlp300, batch_size=FLAG['batch_size'], shuffle=True, num_workers=4)
    ...
    if FLAG['resume'] and os.path.exists(os.path.join(FLAG['images'], "NSLS.pth")):
        state = torch.load(os.path.join(FLAG['images'], "Disout.pth"))
    ...
    for ep in range(start_epoch, target_epoch):
        ...
        #     with torch.no_grad():
        #         origin = cv2.imread("H:/Code/python/PRNet_PyTorch-master/test_data/obama_origin.jpg")
        #         gt_uv_map = np.load("H:/Code/python/PRNet_PyTorch-master/test_data/test_obama.npy")
...
if __name__ == "__main__":
    ...
    for rr in range(r):
        ...
        c = random.random()+0.0000000001
        d = random.uniform(0.00000000000001,0.001)
        # list = [20, 50, 100]
        # i = random.randint(0, 2)
        e = random.uniform(0.00005,0.001)
        # list = [0, 0.1, 0.2, 0.3]
        ...
        parser.add_argument("--train_dir", default='H:/Code/python/PRNet_PyTorch-master/utils/300W-LP/', help="specify input directory.")
        args = parser.parse_args()
        fitness1,fitness2=main(args.train_dir,FLAG)
        print(rr)
        end_time=time.clock()
        fitness3=[]
        fitness3.append(fitness1[-1])
        fitness3.append(fitness2[-1])
        with open('H:/Code/python/PRNet_PyTorch-master/result.csv', 'a+', newline='', encoding='utf-8') as f:
            f_csv = csv.writer(f)
            f_csv.writerow(fitness3)
